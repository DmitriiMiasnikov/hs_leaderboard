...
import { TOption, TReactSelectProps } from './types';
import { getMarginStyles, TMargin } from 'src/shared/types';

export const FieldWrapper = styled.div<TMargin>(
  (props) => css`
    position: relative;
...
export const reactSelectStyles = <T>({
  ...
  control: (styles, { menuIsOpen }) => ({
    ...
    '&::after': {
      ...
      borderTop: menuIsOpen ? 'none' : `0.5rem solid ${theme.colors.grey_600}`,
      borderBottom: menuIsOpen ? `0.5rem solid ${theme.colors.grey_600}` : 'none',
      content: "' '",
    },
  }),
